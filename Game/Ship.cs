...
#region actions
        private void Rotate()
        {
            if (TargetToMove != null)
            {
                ...
                //removeing z axis  
                rotationToTarget = Quaternion.Euler(rotationToTarget.eulerAngles.x , rotationToTarget.eulerAngles.y, zBeforeRotation);
                
                if (p.Rotation!=rotationToTarget)
                {
                    p.Rotation = Quaternion.RotateTowards(p.Rotation, rotationToTarget, p.RotationSpeed * TickDeltaTime/1000f);
